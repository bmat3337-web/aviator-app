...
export function FlightScreen(provider: IGameProvider): HTMLElement {
  ...
  const render=()=>{
    ...
    '<aside class="panel bets"><div class="bet"><div class="bet-head"><span class="bet-name">BET 1</span><span>'+vm.bets.BET1.state+'</span></div><div class="bet-actions"><input class="stake" value="'+(vm.bets.BET1.stake||1)+'" aria-label="Bet 1 stake"><button data-slot="BET1">'+actionLabel(provider.snapshot(),"BET1")+'</button></div></div>'+
    '<div class="bet"><div class="bet-head"><span class="bet-name">BET 2</span><span>'+vm.bets.BET2.state+'</span></div><div class="bet-actions"><input class="stake" value="'+(vm.bets.BET2.stake||1)+'" aria-label="Bet 2 stake"><button data-slot="BET2">'+actionLabel(provider.snapshot(),"BET2")+'</button></div></div><div class="support"><div class="support-row"><span>Environment</span><strong>'+environment.environment+'</strong></div><div class="support-row"><span>Balance</span><strong>SIMULATION</strong></div></div></aside>';
    root.querySelectorAll<HTMLButtonElement>("button[data-slot]").forEach(btn=>btn.onclick=()=>{
      const id=btn.dataset.slot as "BET1"|"BET2", s=provider.snapshot(), b=s.bets[id];
      if(s.round.state==="BETTING_OPEN"&&b.state==="IDLE"){const input=btn.parentElement?.querySelector<HTMLInputElement>("input");provider.placeBet({slot:id,stake:Number(input?.value||1)});}
      else if(s.round.state==="FLYING"&&(b.state==="BET_PLACED"||b.state==="ACTIVE"))provider.cashOut(id);
      render();
    });
  };
