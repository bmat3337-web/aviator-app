...
export function FlightScreen(provider: IGameProvider): HTMLElement {
  ...
  const render=()=>{
    ...
    root.innerHTML='<header class="aviator-header"><button class="menu-button" type="button" aria-label="Open menu">☰</button><div class="header-brand"><span>AVIATOR</span><small>FLY BEYOND LIMITS</small></div><div class="header-balance"><span>$ 250.00</span><button type="button" aria-label="Deposit">+</button></div></header><aside class="panel nav"><div class="brand">AVIATOR</div><div class="nav-item active">Flight</div><div class="nav-item">Challenge</div><div class="nav-item">Social</div><div class="nav-item">History</div><div class="nav-item">Profile</div></aside>'+
    '<section class="panel flight"><div class="flight-top"><div><strong>LIVE FLIGHT</strong><div class="round">'+vm.roundId+' · '+vm.roundState+'</div></div><div class="online">● '+vm.playerCount+' Online</div></div><div class="next-round">NEXT ROUND <strong>00:12</strong></div><div class="multiplier" aria-live="polite">'+vm.multiplier.toFixed(2)+'x</div><div class="sky '+environment.environment+'"></div><div class="trajectory"></div><div class="plane" style="--plane-x:'+String(18+motion.progress*64)+'%;--plane-y:'+String(64-motion.progress*30)+'%;--plane-angle:'+String(motion.angle)+'deg">✈</div><div class="history">'+vm.history.map(x=>'<span class="chip">'+x.toFixed(2)+'x</span>').join("")+'</div></section>'+
