...
export interface ChallengeStore {
  ...
  isSettled(challengeId: string, roundNumber: number): boolean;
  markSettled(challengeId: string, roundNumber: number): void;
}
...
export class InMemoryChallengeStore implements ChallengeStore {
  ...
  predictions(id: string) { return [...this.predictionMap.values()].filter(p => p.challengeId === id); }
  ...
  saveScore(score: ChallengeScore) { this.scoreMap.set([score.challengeId, score.participantId].join(":"), { ...score }); }
  isSettled(id: string, round: number) { return this.settled.has(id + ":" + round); }
  markSettled(id: string, round: number) { this.settled.add(id + ":" + round); }
}

export function submitPrediction(store: ChallengeStore, prediction: ChallengePrediction): void {
...
export function settleRound(store: ChallengeStore, challengeId: string, roundNumber: number, actualState: RoundState): ChallengeScore[] {
  ...
  if (store.isSettled(challengeId, roundNumber)) return store.scores(challengeId);

  for (const prediction of store.predictions(challengeId).filter(p => p.roundNumber === roundNumber)) {
    const existing = store.scores(challengeId).find(s => s.participantId === prediction.participantId);
  ...
  store.markSettled(challengeId, roundNumber);
  const nextRound = roundNumber;
  const nextState = nextRound >= challenge.totalRounds ? "COMPLETED" : "LIVE";
  store.save({ ...challenge, currentRound: nextRound, state: nextState });
